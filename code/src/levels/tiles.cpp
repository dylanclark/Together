...
// render tile
void Tile::render_bg(Camera* cam, int active_color, Texture* tile_tex)
{
    // activity clippers
    int clip_x;
    ...
    tile_tex->render(col_rect.x, col_rect.y, &active_clip, cam);
}

void Tile::render_fg(Camera* cam, int active_color, Texture* tile_tex)
{
    if (m_type == TILE_CLEAR) {
