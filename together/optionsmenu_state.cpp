...
{
    if (!volume_slider.load_object(128, 16, "textures/menu/volume.png", game->rend))
    {
        printf("Failed to load resume button object!\n");
        return;
    }
    
    if (!sfx_slider.load_object(128, 16, "textures/menu/sfx.png", game->rend))
    {
        printf("Failed to load level select button texture!\n");
        return;
    }
    
    if (!controls_button.load_object(96, 16, "textures/menu/controls.png", game->rend))
    {
